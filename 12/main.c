...
int hash_1(ll c1, ll c2);
int hash_2(ll c1, ll c2);
...
StringCode* encode(struct Encoder* encoder, char* s)
{
    ...
    newCode->h1 = hash_1(newCode->c1, newCode->c2);
    newCode->h2 = hash_2(newCode->c1, newCode->c2);
    return newCode;
}
...
int hash_1(ll c1, ll c2)
{
    // printf("%lld %lld %d\n", c1, c2, (int)((c1 + c2) % SLOT));
    return (int)((c1 + c2) % SLOT);
}
